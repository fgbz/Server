...
//        calShift(result);
    ...
    public void getPreView(String fileID, HttpServletResponse response) throws IOException {
        ...
        String fileShortName = attachment.getActualFile().substring(0, attachment.getActualFile().lastIndexOf("."));
        String fileName = fileShortName + ".ofd";
        String serverPath = Attachment.GetFileStorageFolder(fileShortName);
        String fileFullName = serverPath + fileName;
        File file = new File(fileFullName);
        if (!file.exists()) {
            logger.info("getPreView,ofd not exist:" + fileFullName + "\t\t客户端保存文件名filename:" + fileName);
            fileName = fileShortName + ".pdf";
            fileFullName = serverPath + fileName;
            file = new File(fileFullName);
            if (!file.exists()) {
                logger.info("getPreView,pdf not exist:" + fileFullName + "\t\t客户端保存文件名filename:" + fileName);
                return;
            }
        }

        DownloadAttachmentForPreview(fileFullName, fileName, response);
        logger.info("getPreView,开始下载文件,全文件名:" + fileFullName + "\t\t客户端保存文件名filename:" + fileName);
    }

    private void DownloadAttachmentForPreview(String fileFullName, String fileName, HttpServletResponse response) {
        Assert.hasLength(fileFullName);
        ...
        try {
            ...
            logger.info("download file faild:" + e.toString());
            e.printStackTrace();
        }
...
//		file.deleteOnExit();
    ...
    private FileState saveWithNoThum(String userid, String module, HttpServletRequest request) {
        ...
        try {
            ...
            logger.info("upload filelength：\t" + multipartFile.getSize());
