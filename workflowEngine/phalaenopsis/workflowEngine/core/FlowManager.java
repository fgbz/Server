...
import phalaenopsis.workflowEngine.storage.WFBranchRoute;
...
public class FlowManager {
	 ...
	 * @return
	 */
	public WFInstance start(IWorkflowMap map, InstanceRuntimeContext context) {
		// 创建一个新的流程实例数据
		...
		obj.setState(WFInstanceState.Created);
	...
	public String next(IWorkflowInstance instance) throws CannotFlowException {
		...
		if (flowContext.State == FlowState.RedirectNew) {
			// 拦截器重定向新节点
		...
		if (obj.getState() == WFInstanceState.Created)
			obj.setState(WFInstanceState.Running);
	 ...
	 */
	private String tryDealState(NodeFlowContext flowContext, IWorkflowInstance instance) {
		String result = null;
		switch (flowContext.State) {
		case FlowState.Break:
			result = null;
			return result;
	 ...
	 * @throws Exception
	 */
	public String previous(IWorkflowInstance instance) throws Exception {
		if (instance.getFinished())
			throw new CannotFlowException("流程已经结束。");

		NodeFlowContext flowContext = new NodeFlowContext(instance);
		flowContext.flowType = FlowType.Previous;
	...
	private static void flow(IWorkflowInstance instance, NodeInfo node, NodeFlowContext flowContext)
		...
		// 创建流程节点实例，并且设置NodeID
		IWorkflowNode current = (IWorkflowNode) node.NodeType.newInstance();
		current.setNodeID(node.getNodeID());

		// 通知当前节点流程即将流转
		if (flowContext.flowType == FlowType.Next) {
			current.onNext(flowContext);
		} else if (flowContext.flowType == FlowType.Previous) {
	 ...
	 * @throws Exception
	 */
	private static NodeInfo getNextNode(NodeInfo current, IWorkflowInstance instance) throws Exception {
		// End节点之后的下一个节点被忽略
		if (current.isIsEnd()) {
		...
		if (current.IsBranch) {
			// 是分支节点
			// 获取分支选择器
			Assert.notNull(current.BranchSelector);
			IBranchSelector selector = (IBranchSelector) current.BranchSelector.newInstance();
	 ...
	 * @return
	 */
	private int getLastSequence(IWorkflowInstance instance) {
	 ...
	 * @return
	 */
	private WFHistory getHistory(IWorkflowInstance instance, int sequence) {
	 ...
	 * @param instance
	 */
	public void saveFinishedData(IWorkflowInstance instance) {
		WFInstance obj = instance.getContext().getInstanceData();
		obj.setNodeID(instance.getMap().getFinishedNode());
		obj.setState(WFInstanceState.Finished);
		int lastSequence = getLastSequence(instance);
		int currentSequence = lastSequence + 1;
		obj.setSequencePtr(currentSequence);
