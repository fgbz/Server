 ...
 * 	
 */
package phalaenopsis.workflowEngine.configuration;
...
import phalaenopsis.workflowEngine.core.WorkflowMap;
import phalaenopsis.workflowEngine.core.WorkflowMapCollection;
import phalaenopsis.workflowEngine.core.WorkflowNodeType;
...
public class WorkflowsConfigurationLoader {
	...
	// /// 从配置信息中加载流程图
	// /// </summary>
	// /// <param name="config">特定的配置信息，如配置文件的路径、配置信息字符串、配置节名称等</param>
	// /// <returns></returns>
	// public WorkflowMapCollection LoadFile(String config) {
	// try {
	// Resource resource = new ClassPathResource("conf/workflow-config.xml");
	// File file = resource.getFile();
	// //该方法的参数config就是下面的path传递过来的
	// String path = file.getPath();
	// Document document = loadAndVerifyDocument(path);
	// WorkflowMapCollection mapCollection;
	// mapCollection = analyzeDocument(document);
	...
	// }
	//
	// public ICollection<IWorkflowMap> LoadXml(string xml)
	// {
	// XmlDocument document = new XmlDocument();
	// document.LoadXml(xml);
	// return new WorkflowMapCollection();
	// }
	/// <summary>
	/// 从配置信息中加载流程图
	/// </summary>
	...
	private Document loadAndVerifyDocument(String config) {
		...
		try {
			Resource resource = new ClassPathResource("conf/WorkflowEngine-schema.xsd");
	...
	private WorkflowMapCollection analyzeDocument(Document document) throws Exception {
		...
		for (int i = 0; i < workFlowList.getLength(); i++) {
			...
			WorkflowMap map = new WorkflowMap(mapID, keep);
			// nodes
			...
			for (int j = 0; j < nodeList.getLength(); j++) {
				...
				int nodeType = WorkflowNodeType.Default;
				...
				try {
					map.addNode(nodeID, GetClassType(classType), nodeType);
				} catch (Exception e) {
					throw e;
				}
			}
	 ...
	 * @return
	 */
	public int getWorkFlowNodeType(String workNodeType) {
		int workFlowNodeType = WorkflowNodeType.Default;
		switch (workNodeType) {
		case "start":
			workFlowNodeType = WorkflowNodeType.Start;
			break;
		case "end":
			workFlowNodeType = WorkflowNodeType.End;
			break;
		default:
			workFlowNodeType = WorkflowNodeType.Default;
			break;
		}
